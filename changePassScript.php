...
if ((isset($_GET['email']) ) && (isset($_GET['hash']))){
	...
	if ( ($row['email'] == $email) && ($row['hash'] == $hash) ) {
		...
		$_SESSION['uname'] = $row['uniqueUsername'];

	}

}

else if (isset($_POST['ResetPassSubmit'])){
	
	$unhashedpassword_phrase = test_input($_POST["psw"]);
	$password_phrase = md5($unhashedpassword_phrase);
	
	$uniqueUsername = $_SESSION['uname'];
	
	//echo "change Password for".$_SESSION['uname']."<br>";
	//echo "Password :".$unhashedpassword_phrase."<br>";
	//echo "Password Hashed:".$password_phrase."<br>";
	
	$sql = "UPDATE `USERDATA` SET `password`='$password_phrase' WHERE `uniqueUsername`= '$uniqueUsername'";
	//mysqli_query($con, $sql);
	
	if ($con->query($sql) === TRUE) {
		    $_SESSION['uniqueUsername'] = $_SESSION['uname'];
...
<html>
				...
				<div class="search-box">
					...
					<div class="result"></div>
						...
						if (isset($_SESSION['uniqueUsername'])){
							...
							echo '<a href="profile.php?'.$_SESSION['uniqueUsername'].'">Home</a> ';
    ...
    <form id="signup" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
        ...
        <h1 class="reg_title">Reset Password</h1>
        ...
        <div class="reg_form">
            ...
            <input type="submit" ID="submitbtn" name="ResetPassSubmit" value="Submit"/>
        </div>
